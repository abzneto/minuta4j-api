; (function() {
    ...
    window.minuta4j = {
        ...
        postJSON: function (url, options) {
            var xhr = minuta4j.createRequest('POST', url, {async: true});
            ...
            if (options.data) {
                var string = JSON.stringify(options.data);
        ...
        loadJSON: function(url) {
            return minuta4j.syncRequestJSON('OPTIONS', url);
        },
        ...
        requestAJAX: function(settings ) {
            var jqXHR = minuta4j.createRequest(settings.method, settings.url, {async: settings.async});
        ...
        uploadPhoto: function(settings) {
            var xhr = minuta4j.createRequest('POST', settings.url, {async: settings.async});
